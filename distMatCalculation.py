...
for i in range(4999):
    for j in range(i + 1, 5000):
        distMatTfidf[i][j] = calcNormal(setSet[i], setSet[j], mSet[i], mSet[j], tfidfVect[i].vector,
                                        tfidfVect[j].vector)
        if (z % 100000 == 0):
            print(z)
        z += 1
...
for i in range(4999):
    for j in range(i + 1, 5000):
        distMatPlain[i][j] = calcOkapi(setSet2[i], setSet2[j], avgBytes, bytesInDoc[i], overallDoc, k1, b, k2,
                                       plainVect[i].vector, plainVect[j].vector)
        if (z % 100000 == 0):
            print(z)
        z += 1
